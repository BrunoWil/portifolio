...
async fn carregar_portfolio() -> Result<(), JsValue> {
    ...
    for proj in data.projetos {
        ...
        let card = format!(
            ...
            proj.categoria, proj.titulo, proj.descricao, tags_html
        );
        projetos_html.push_str(&card);
    }

    let date = js_sys::Date::new_0();
    let ano = date.get_full_year().to_string();

    // 3. Montar a estrutura HTML principal (sem onclick inline)
    let body_html = format!(
        r##"
        <nav>
            <a href="#inicio" class="logo nav-scroll">{}.</a>
            <div class="nav-links">
        ...
        <section id="inicio">
            ...
            <p>{}</p>
            <a href="#projetos" class="btn nav-scroll">Ver Meus Trabalhos</a>
        </section>
        <section id="projetos">
    ...
    let nav_links = document
        .query_selector_all(".nav-scroll")
        .map_err(|_| JsValue::from_str("Erro ao buscar links de navegação"))?;

    for i in 0..nav_links.length() {
        if let Some(node) = nav_links.get(i) {
            let element: web_sys::HtmlElement = node
                .dyn_into()
                .map_err(|_| JsValue::from_str("Erro ao converter nó"))?;
            ...
            element
                ...
                .map_err(|_| JsValue::from_str("Erro ao adicionar listener"))?;
